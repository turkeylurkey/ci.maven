...
public class DevMojo extends StartDebugMojoSupport {
    ...
    protected void doExecute() throws Exception {
        ...
        if (isUsingBoost()) {
            ...
            Set<String> visibleLibertyProjectDependencies = new HashSet<String>(libertyProjectDependencies);
            visibleLibertyProjectDependencies.retainAll(features);
            log.warn("maven dependencies that are VALID liberty features:"+visibleLibertyProjectDependencies);
            /*protected static final*/ String PLUGIN_ADDED_FEATURES = "configDropins/overrides/liberty-plugin-added-features.xml";
            File newServerXml = new File(serverDirectory, PLUGIN_ADDED_FEATURES);
            log.warn("New server xml file:"+newServerXml);
            newServerXml.delete(); // about to regenerate. Not needed during getLibertyDirectoryPropertyFiles

            Map<String, File> libertyDirPropertyFiles = BasicSupport.getLibertyDirectoryPropertyFiles(installDirectory, userDirectory, serverDirectory);
            // existingFeatures are all lowercase
            Set<String> existingFeatures = servUtil.getServerFeatures(serverDirectory, libertyDirPropertyFiles);
            ...
            log.warn("maven dependencies that are VALID liberty features but are missing:"+missingLibertyFeatures);

            // Create server-devmode.xml
